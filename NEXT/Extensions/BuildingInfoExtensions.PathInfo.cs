...
namespace MetroOverhaul.NEXT.Extensions
{
    public static partial class BuildingInfoExtensions
    {
        ...
        {
            var netInfo = info.m_finalNetInfo;
            if ( netInfo.IsAbovegroundMetroStationTrack())
            {
                ...
                //    info.AssignNetInfo(trackName, false);
                //}
                var newNetInfoName = Util.GetMetroStationTrackName(netInfo.GetNetInfoVersion(), style, netInfo.DeduceTrackType());
                info.AssignNetInfo(newNetInfoName);
            }
        }
    }
}
